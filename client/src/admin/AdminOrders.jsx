...
const AdminOrders = () => {
    ...
    const getOrders = async () => {
        try {
            const { data } = await axios.get(`http://localhost:5000/api/user/allOrders`)
            setOrder(data)
        } catch (err) {
    ...
    const handleChange = async (orderId, value) => {
        try {
            const { data } = await axios.put(`http://localhost:5000/api/user/orderStatus/${orderId}`, { status: value, });
            getOrders();
